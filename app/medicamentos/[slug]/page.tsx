...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title, description, url: `https://genericoou.online/medicamentos/${med.slug}/`, type: "article" },
  };
}
    ...
    <>
      ...
      <div className="container section">
        ...
        <div className={styles.header}>
          <div>
            <div className="flex flex-wrap items-center gap-2" style={{ marginBottom: ".75rem" }}>
              ...
              {med.bioequivalenciaAnvisa && <span className="badge badge-green">✓ Bioequivalente ANVISA</span>}
            </div>
            <h1 className={styles.title}>Genérico de <strong>{med.referencia}</strong></h1>
          ...
          <div className={styles.savingsBox}>
            ...
            <p className={styles.savingsNote}>*Estimativa baseada em comparações de mercado. Preços variam por farmácia.</p>
          </div>
        </div>
        ...
        <div className={styles.grid}>
          ...
          <section className={styles.genericosSection}>
            <h2 className={styles.sectionH2}>
              ...
              <span style={{ fontWeight: 400, fontSize: ".85rem", color: "var(--text-muted)", marginLeft: ".5rem" }}>({med.genericos.length})</span>
            </h2>
            <ul className={styles.genericosList}>
              {med.genericos.map((g, i) => (
                <li key={i} className={styles.genericoItem}>
                  ...
                  {med.bioequivalenciaAnvisa && <span className="badge badge-green" style={{ fontSize: ".7rem" }}>✓ ANVISA</span>}
                </li>
              ))}
            </ul>
          </section>
        ...
        <div className={styles.infoBox}>
          ...
          <p>Os medicamentos genéricos listados são <strong>bioequivalentes</strong> ao medicamento de referência <strong>{med.referencia}</strong>, o que significa que possuem a mesma eficácia terapêutica, segurança e qualidade, comprovadas por estudos aprovados pela ANVISA.</p>
          <p style={{ marginTop: ".75rem" }}>Ao comprar, verifique a <strong>faixa amarela</strong> na embalagem — identificação obrigatória dos genéricos no Brasil. Sempre consulte seu médico ou farmacêutico antes de substituir medicamentos.</p>
        </div>
        <div style={{ marginTop: "2.5rem" }}>
          <Link href="/medicamentos/" className="btn btn-outline" style={{ marginRight: "1rem" }}>← Ver todos os medicamentos</Link>
          <Link href="/como-funciona-generico/" className="btn btn-outline">Entender bioequivalência →</Link>
        </div>
